    ...
    <div class="content">
        ...
        <Modal v-model="modalStatus" :title="modalTitle" width="600" @on-cancel="handleReset" :mask-closable="false">
            <Form ref="formItem" :model="formItem" :label-width="120" :rules="ruleValidate">
                ...
                <FormItem label="最小净买入量" prop="minBuyIn">
                    <Input v-model="formItem.minBuyIn" style="width:200px" placeholder="请输入最小净买入量" number></Input>
                </FormItem>
                <FormItem label="最小持仓量" prop="minPosition">
                    <Input v-model="formItem.minPosition" style="width:200px" placeholder="请输入最小持仓量" number></Input>
                </FormItem>
                ...
                <FormItem label="奖励发放形式">
                    <Checkbox v-model="formItem.autoReward" :disabled="true" :true-value="1" :false-value="0">手工发放奖励（交易活动目前只支持手动发放奖励）</Checkbox>
                </FormItem>
            </Form>
            <div slot="footer">
    ...
    export default {
        ...
        data () {
            ...
            return {
                ...
                ruleValidate: {
                    ...
                    minBuyIn: [
                        {required: true, type: 'number', message: '请输入最小净买入量', trigger: 'blur'},
                        {validator: numberPositive, trigger: 'blur'}
                    ],
                    minPosition: [
                        {required: true, type: 'number', message: '请输入最小持仓量', trigger: 'blur'},
                        {validator: numberPositive, trigger: 'blur'}
                    ],
                    startTime: [
        ...
        methods: {
            ...
            showEditModal (currRow, type) {
                this.editType = type;
                if (type == 0) {
                    this.modalTitle = '新增交易活动';

                    // 添加
                    this.setFormItem();
                } else if (type == 1) {
                    this.modalTitle = '编辑交易活动';
            ...
            setFormItem (row) {
                if (row) {
                    ...
                    this.formItem.minBuyIn = row.minBuyIn;
                    this.formItem.minPosition = row.minPosition;
